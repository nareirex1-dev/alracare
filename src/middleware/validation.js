...
const { sanitizePhone, isValidPhone } = require('../utils/helpers');
...
const validateServiceId = (req, res, next) => {
  try {
    ...
    // Kita ganti isValidUUID dengan validasi string yang ketat
    // agar sinkron dengan ID manual di database.sql
    ...
    // Opsional: Jika ingin lebih kuat, cek apakah ID mengandung tanda hubung
    if (!id.includes('-')) {
       return res.status(400).json({ 
         success: false,
         message: 'Format ID Layanan salah (contoh: layanan-01)'
       });
    }
    ...
    logger.error('Validation error:', { error: error.message });
    res.status(400).json({ success: false, message: 'Validasi gagal' });
  }
};
...
// Import logger for error handling within middleware
