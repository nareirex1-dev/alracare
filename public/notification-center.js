...
async function loadNotifications(phone, filter = 'all') {
    try {
        ...
        if (response.success) {
            ...
            unreadCount = countResponse.success && countResponse.data ? (countResponse.data.count ?? 0) : 0;
...
function startNotificationAutoRefresh(phone, filter) {
    ...
    notificationInterval = setInterval(async () => {
        try {
            ...
            if (response.success) {
                const countResponse = await apiCall(API_CONFIG.ENDPOINTS.NOTIFICATIONS_UNREAD_COUNT(phone));
                const newUnreadCount = countResponse.success && countResponse.data ? (countResponse.data.count ?? 0) : 0;
